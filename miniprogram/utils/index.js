 ...
 */
const getLunarDate = function (year, month, day) {
  ...
  yearCyl = iYear - 1864;
  leapMonth = leapMonthFun(iYear); // 闰哪个月,1-12
  var leap = false; // 默认值
  ...
  var newmonth = data.chineseNumber[iMonth - 1];
  return {
    month: newmonth + '月',
    day: offset + 1
  };
...
const yearDays = function (y) {
  var i, sum = 348;
  for (i = 0x8000; i > 0x8; i >>= 1) {
    if ((data.lunarInfo[y - 1900] & i) != 0)
      sum += 1;
  }
...
const leapDays = function (y) {
  ...
  if (leapMonthFun(y) != 0) {
    if ((data.lunarInfo[y - 1900] & 0x10000) != 0)
      return 30;
    else
      return 29;
  } else
...
const leapMonthFun = function (y) {

  return data.lunarInfo[y - 1900] & 0xf;
}
const monthDays = function (y, m) {
  if ((data.lunarInfo[y - 1900] & (0x10000 >> m)) == 0)
    return 29;
  else
    return 30;
}
const getChinaDayString = function (day) {
  ...
  if (day == 10)
    ...
    return chineseTen[parseInt(day / 10)] + data.chineseNumber[n];
}
